...
while($row = mysql_fetch_array($result_rptrecord)){
	if ($row['ip6']) {
		$ip_addr = inet_ntop($row['ip6']);
	}
	else {
		$ip_addr = long2ip($row['ip']);
	}
	...
	echo "<tr align=center bgcolor=". $rowcolor. ">";
        echo "<td><a name=rpt". $row['serial'].">". $ip_addr. "</td><td>". gethostbyaddr($ip_addr). "</td><td>". $row['rcount']. "</td><td>". $row['disposition']. "</td><td>". $row['reason']. "</td><td>". $row['dkimdomain']. "</td><td>". $row['dkimresult']. "</td><td>". $row['spfdomain']. "</td><td>". $row['spfresult']. "</td>";
        echo "</tr>";
	echo "\n";
}
echo "</table>";
